...
class SQLUtils:
    ...
    class Connection:
        ...
        def is_table_exists(self, table_name: str) -> bool:
            if self._base_type == SQLUtils._BaseType.ACCESS:
            ...
            elif self._base_type == SQLUtils._BaseType.POSTGRES:
        ...
        def connect_to_mdb(base_path):
            connection_string: str = 'Driver={{Microsoft Access Driver (*.mdb, *.accdb)}};DBQ={0};'. \
                format(base_path)
            connection: SQLUtils.Connection = SQLUtils.Connection(connection_string)
            connection._base_type = SQLUtils._BaseType.ACCESS
            return SQLUtils.Connection(connection_string)
        ...
        def connect_to_postgre(database: str, user: str, password: str, server: str, port: int):
            ...
            connection: SQLUtils.Connection = SQLUtils.Connection(connection_string)
            connection._base_type = SQLUtils._BaseType.POSTGRES
            return SQLUtils.Connection(connection_string)
